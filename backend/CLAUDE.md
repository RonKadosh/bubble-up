...
- **`LoadTestSeeder` (`bootstrap/seed/`) bulk-seeds a full-semester dataset for DB load testing.** Toggle `app.loadtest.enabled` (default OFF; env `APP_LOADTEST_ENABLED`), volumes in `app.loadtest.*` (`LoadTestProperties`). Defaults: 10k users / 500 `LT-####` courses+offerings in the **current** term / ~40k enrollments / ~5k groups / ~100k backdated chat messages (one forced 1000-message hot group) / polls+pins+calendar — then runs the **real** `refreshUserMatchCache` per user (≈300k cache rows; the slow phase, minutes — it re-resolves course/offering per candidate group per user). Inert direct `EntityManager` writes (same seed-time module-boundary exception as DemoSeeder/MatchingScenarioSeeder) — no events fire; timestamps are explicitly backdated across the term window. Idempotency anchor = `loadtest-0@bubble.up`; a partially failed run requires a DB wipe before re-running. Dev-only tool — never enable where real users exist (shared password, synthetic catalog).
- **Catalog course search uses a leading-wildcard `LIKE` with no trigram index in prod.** `CourseRepository.search` (powers both the public `GET /api/catalog/courses/search` and the admin course search) does `lower(code/name) like '%q%'`, which a B-tree can't use — Postgres full-scans `courses` per keystroke. Capped at 50 rows so it's bounded, and fine at a few-thousand-course catalog, but sloppy at a full multi-year university catalog under concurrency. Prod fix (Postgres-only, **not** a JPA `@Index`, no Flyway yet so apply by hand): `CREATE EXTENSION IF NOT EXISTS pg_trgm; CREATE INDEX idx_courses_code_trgm ON courses USING gin (lower(code) gin_trgm_ops); CREATE INDEX idx_courses_name_trgm ON courses USING gin (lower(name) gin_trgm_ops);`. H2 tests are unaffected (unindexed at test scale is harmless).
- **`common/validation/` and `common/utils/`** were referenced in older docs but don't exist. Don't create them speculatively.
- **Matching v1 schema is dev/test only (H2 ddl-auto create-drop).** Prod Postgres needs: `ALTER TABLE user_profiles ADD COLUMN meaningful_behavior_events INT NOT NULL DEFAULT 0`; on `group_profiles` add `group_profile_confidence DOUBLE PRECISION NOT NULL DEFAULT 0`, `trending_activity_count BIGINT NOT NULL DEFAULT 0`, `trending_recent_joins INT NOT NULL DEFAULT 0`, `trending_upcoming_sessions INT NOT NULL DEFAULT 0`; on `user_match_cache` add `matching_confidence DOUBLE PRECISION NOT NULL DEFAULT 0` and `match_percent INT NULL`. The term-scoping change additionally needs `ALTER TABLE user_match_cache ADD COLUMN offering_id UUID NULL` (term-precise key for a future term-rollover sweep / partition; populated on every cache rebuild). The behavior-signals redesign additionally **drops** the 7 `user_profiles.behavior_*_score` columns and **adds** the `@ElementCollection` table `CREATE TABLE user_profile_behavior_counts (user_profile_id UUID NOT NULL, event_type VARCHAR(64) NOT NULL, count INT NOT NULL)` with index `idx_behavior_counts_profile (user_profile_id)`. Same "no Flyway yet" caveat as the chat-polish note above. (The v1 redesign also dropped the `app.matching.credibility-threshold` and `targets` config — replaced by `matched-display-threshold`, `confidence`, and `trending` blocks; the signals redesign replaced the flat `app.matching.deltas` block with `app.matching.signals`.)
- **Matching v1 = complementarity + confidence blend.** `MatchingScorer` scores `final = matching_confidence·cosine(user, 1−group_avg) + (1−matching_confidence)·trending`, where `matching_confidence = user_confidence · group_profile_confidence`. Ranking uses `final_score`; the displayed `%` is the raw cosine (only for MATCHED, i.e. `matching_confidence ≥ matched-display-threshold`). Trending uses only existing signals (chat+file activity, recent joins, member count, upcoming GROUP calendar events).
- **Behavior role vector = per-action saturation, normalized.** A user's behavior vector is derived (not stored) from raw per-action-type counts (`user_profile_behavior_counts`): each action contributes `weight·(count/(count+k))` to its role(s) per `app.matching.signals`, then the per-role sums are **max-normalized** to a shape (`MatchingScorer.behaviorVector`) — mirroring the quiz vector. This is what prevents heavy usage from squashing every role to 1.0, and the per-action `k` (large for chat) keeps a frequent action from dominating the shape. **Wiring is config-driven**: adding a signal = new `BehaviorEventType` value + one `eventPublisher.publishEvent(new UserBehaviorEvent(userId, TYPE))` at the action's choke point + one `app.matching.signals` entry (no scorer change). `REACTED_TO_MESSAGE` (reactions are frontend-only) and `STARTED_VIDEO_CALL` (only call presence exists) remain unwired placeholders awaiting backend features.
- **Behavior confidence = diversity, not volume (anti-spam).** `behavior_confidence = min(behavior_evidence / behaviorEvidenceCap, 1)` where `behavior_evidence = Σ over action types of count/(count+k)` (`MatchingScorer.behaviorEvidence`). Each action type saturates below 1, so spamming ONE action (e.g. chat) can't inflate profile strength — only a SPREAD of distinct actions raises it. `meaningful_behavior_events` is now telemetry only (it no longer feeds confidence). Config knob `app.matching.confidence.behavior-evidence-cap` (default 3.0) replaced the old event-count `behavior-cap`.
- **Quiz weights lean toward the behavior-blind roles.** Because Creative, TeamPlayer, and Challenger have weak/no behavior signals, `MatchingDataSeeder` weights them at primary 0.20 / secondary 0.10 vs 0.12 / 0.06 for the behavior-observable roles (Leader, Planner, Expert, Communicator) — so the quiz carries the roles behavior can't. The seeder is idempotent (only seeds when the quiz table is empty); admins can further edit via the AdminQuiz surface.
- **Matching feedback is the validation harness — `matchingfeedback/` module.** It accumulates, per `(user, group)` in one upserted `match_feedback` row, the funnel that lets you actually measure the engine: impression (shown as MATCHED/TRENDING, with `match_percent`), outcome (`joined_at`), and an explicit `rating` (GOOD_FIT/BAD_FIT from the group view, stamped with the match % resolved at click time). Wiring reuses the existing event pattern — `RecommendationsShownEvent` (from `MatchingQueryService.getRecommendations`) and `GroupJoinedEvent` (from `GroupCommandService.joinGroup`, carries `(userId, groupId)` because neither `GroupMembershipChangedEvent` nor `UserBehaviorEvent.JOINED_GROUP` carries both) → `@Async` `MatchFeedbackEventListener`. Explicit ratings come in via `POST /api/matching/feedback`; the % is resolved by the new `MatchingInternalService.matchSnapshotFor` (cache row if a candidate, else scored live — members aren't in their own candidate cache). Reflection is SQL `GROUP BY` behind admin-gated `GET /api/admin/matching-feedback` (funnel by mode, join-rate by match-% bucket, rating totals). **No big-data dep — Postgres + JPA + Spring events**, by design. **Schema is dev/test H2 create-drop**: prod Postgres needs `CREATE TABLE match_feedback (...)` with unique `(user_id, group_id)`; same "no Flyway yet" caveat.
- **Onboarding state schema is dev/test only (H2 ddl-auto create-drop).** Prod Postgres needs `CREATE TABLE onboarding_state (id UUID PRIMARY KEY, user_id UUID NOT NULL UNIQUE, collapsed BOOLEAN NOT NULL DEFAULT FALSE, wizard_level INT NOT NULL DEFAULT 1, updated_at TIMESTAMP WITH TIME ZONE NOT NULL)` + the `@ElementCollection` table `CREATE TABLE onboarding_acknowledged (onboarding_state_id UUID NOT NULL REFERENCES onboarding_state(id), ack_key VARCHAR(64) NOT NULL)` with index `idx_onboarding_ack_state (onboarding_state_id)`. Same "no Flyway yet" caveat as the chat/matching notes above. The `onboarding/` module is otherwise stateless — step completion is derived live from other modules' `internal/` services; this table holds only the user's own UI acknowledgements (`acknowledged` is an open, namespaced key set — `explainer:*`, `guide:*` — so new explainers/guides need no schema change), the widget collapse flag, and `wizard_level` (the gating "Getting started" wizard's current level; 1–5, 6 = finished).
