            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {filteredExperts.map((e) => (
                <li key={e.id}>
                  <Link
                    to={`/experts/${e.userId}`}
                    className="block bg-surface rounded-xl border border-line p-4 hover:border-primary-500 transition"
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <h2 className="text-base font-semibold text-base">{e.headline}</h2>
                      {e.verificationStatus === 'VERIFIED' && (
                        <span className="text-xs px-2 py-0.5 rounded-full bg-success/15 text-success">{t('expert.directory.verifiedBadge')}</span>
                      )}
                    </div>
...
function OpenSessionCard({ session, ownedGroups, onEnrolled }: OpenSessionCardProps) {
  ...
  return (
    <li className="bg-surface rounded-xl border border-line p-4">
            ...
            <div className="flex flex-col gap-1 max-h-32 overflow-y-auto">
              ...
              {ownedGroups.map((g) => (
                <button
                  ...
                  disabled={busy}
                  className="text-xs px-2 py-1 rounded-full border border-line text-base hover:bg-surface-hover disabled:opacity-50 transition"
                >
                  {g.name}
                </button>
              ))}
